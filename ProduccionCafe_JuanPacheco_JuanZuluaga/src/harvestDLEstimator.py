...
class HarvestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        img_path = os.path.join(
            self.img_dir, self.data.iloc[idx]["class"], self.data.iloc[idx]["filename"]
        )
        ...
        return (
            image,
            torch.tensor([class_label], dtype=torch.float32),
            torch.tensor([days_to_harvest], dtype=torch.float32),
        )
...
class CNNTrainer:
    ...
    def create_data_loaders(self, batch_size=16):
        ...
        full_dataset = HarvestDataset(
            self.csv_file, self.img_dir, transform=val_transform
        )
    ...
    def objective(self, trial):
        ...
        model = HarvestCNN(
            dropout_rate=params["dropout_rate"], num_filters=params["num_filters"]
        )
        ...
        for epoch in range(50):
            ...
            val_loss, val_r2, val_mae, _, _ = self.validate(
    ...
    def train_best_model(self, epochs=100):
        ...
        for epoch in range(epochs):
            ...
            if epoch % 10 == 0:
                print(
                    f"Epoch {epoch}: Train Loss: {train_loss:.4f}, Val Loss: {val_loss:.4f}, Val R²: {val_r2:.4f}"
                )
        ...
        test_loss, test_r2, test_mae, test_preds, test_actual = self.validate(
